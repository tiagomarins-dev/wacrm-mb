-- ============================================================
-- 072: filtro "Agente de IA" no Pulso. p_agent_id = sentinela da IA
-- (ou id de perfil de IA) → conversas com participação do bot;
-- FRT/SLA viram pareamento cliente→bot; vendas do funil = sem humano.
-- Reaplica pulse_tiles/timeline/breakdowns/radar (pulse_ai_row inalterada).
-- ============================================================
...
begin
  ...
  -- Filtro "Agente de IA": sentinela (constants.ts) ou id de perfil de IA →
  -- filtra por PARTICIPAÇÃO do bot na conversa (não por assigned — a IA
  -- transfere/fecha e perde a atribuição; participação espelha pulse_ai_row).
  is_ai := p_agent_id is not null and (p_agent_id = '00000000-0000-0000-0000-0000000000a1'
           or exists (select 1 from ai_profiles ap where ap.account_id = acc and ap.id = p_agent_id));
  ...
  where c.account_id = acc
    ...
    and (p_agent_id is null
         or (not is_ai and c.assigned_agent_id = p_agent_id)
         or (is_ai and exists (select 1 from messages mb where mb.conversation_id = c.id and mb.sender_type = 'bot')));
  ...
  where s.resp_agent is not null
    and (p_agent_id is null or (not is_ai and s.resp_agent = p_agent_id));

  -- FRT/SLA da IA: pareamento cliente→bot (clock time, cap 240 — espelha pulse_ai_row)
  if is_ai then
    with msgs as (
    ...
    'funil', jsonb_build_object(
      ...
      'fechados', (
        ...
        where s.account_id = acc and s.status = 'confirmed'
          ...
          and (p_agent_id is null or (not is_ai and s.atendente_id = p_agent_id) or (is_ai and s.atendente_id is null))
      )
    )
  );
end;
