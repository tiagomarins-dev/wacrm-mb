-- ============================================================
-- Filtro de data (last_message_at) na busca de /conversations. Estende a
-- search_conversations (mig 060): + p_date_from / p_date_to. A assinatura
-- muda → DROP + CREATE (evita overload ambíguo no PostgREST) + re-grant.
-- Mantém SECURITY DEFINER + guard de conta (auth.uid()) da 060. O índice
-- (account_id, last_message_at desc) da 060 já serve o range + ordenação.
-- Idempotente (drop if exists / create or replace). Reverter = reaplicar 060.
-- ============================================================
drop function if exists search_conversations(text,text,uuid,boolean,uuid,int,int);
...
alter function search_conversations(text,text,uuid,boolean,uuid,timestamptz,timestamptz,int,int) owner to postgres;
grant execute on function search_conversations(text,text,uuid,boolean,uuid,timestamptz,timestamptz,int,int) to authenticated;
revoke execute on function search_conversations(text,text,uuid,boolean,uuid,timestamptz,timestamptz,int,int) from anon;
