-- ============================================================
-- 069: rastro de mudança de status de conversa + RPCs da página Pulso (F1).
-- Pré-requisito no app: mergeThread filtra type='status_changed' (fix C1)
-- ANTES de aplicar esta migration (o trigger emite realtime na hora).
-- ============================================================

-- 1) Colunas novas em conversation_events (padrão 048: sem FK em referência fraca)
alter table conversation_events add column if not exists from_status text;
alter table conversation_events add column if not exists to_status text;

-- 2) CHECK += 'status_changed' — nome default do CHECK inline de 048:13
...
alter table conversation_events add constraint conversation_events_type_check
  check (type in ('assigned','transferred','unassigned','status_changed'));

-- Índice p/ pulse_tiles (resolução: busca por conta+tipo+janela)
create index if not exists idx_conversation_events_acc_type_created
  on conversation_events (account_id, type, created_at);

-- 3) Trigger de status (espelha log_conversation_assignment_change, 048)
-- actor_user_id = auth.uid(): quem fechou/reabriu; null = sistema (cron/automação via service-role)
create or replace function log_conversation_status_change()
returns trigger language plpgsql security definer set search_path = public as $$
begin
  if new.status is distinct from old.status then
    insert into conversation_events (account_id, conversation_id, type, from_status, to_status, actor_user_id)
    values (new.account_id, new.id, 'status_changed', old.status, new.status, auth.uid());
  end if;
  return new;
end;
$$;
drop trigger if exists trg_log_conversation_status on conversations;
create trigger trg_log_conversation_status
  after update of status on conversations
  for each row execute function log_conversation_status_change();

-- 4) RPC pulse_tiles — 5 métricas do topo. Admin-only (molde 054).
...
begin
  select account_id into acc from profiles where user_id = auth.uid();
  if acc is null or not is_account_member(acc, 'admin') then return null; end if;
  p_from := greatest(p_from, p_to - interval '400 days');
