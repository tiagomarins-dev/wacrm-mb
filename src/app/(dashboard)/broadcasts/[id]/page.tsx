...
export default function BroadcastDetailPage() {
  ...
  useEffect(() => {
    ...
    fetchData();
    // t entra só no fallback de erro; não re-buscar ao trocar idioma.
  ...
  function handleExport() {
    ...
    const header = [
      ...
      t('detail.csvStatus'),
      t('detail.csvSentAt'),
      t('detail.csvDeliveredAt'),
      t('detail.csvReadAt'),
      t('detail.csvRepliedAt'),
      t('detail.csvError'),
    ];
  ...
  async function handleDelete() {
    ...
    setDeleting(false);
    if (delErr) {
      toast.error(t('detail.deleteError', { message: delErr.message }));
      return;
    }
  ...
  const funnelSteps: FunnelStep[] = [
    ...
    { label: t('detail.funnelDelivered'), value: broadcast.delivered_count, color: 'bg-teal-500' },
    { label: t('detail.funnelRead'), value: broadcast.read_count, color: 'bg-blue-500' },
    { label: t('detail.funnelReplied'), value: broadcast.replied_count, color: 'bg-indigo-500' },
  ];
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-3 text-sm text-muted-foreground">
              ...
              <span>-</span>
              <span>
                {t('detail.createdAt', { date: formatDate(broadcast.created_at) })}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <StatCard
          label={t('detail.statTotalRecipients')}
      ...
      <FunnelChart steps={funnelSteps} title={t('detail.funnel')} />
      ...
      <div className="rounded-xl border border-border bg-card">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-4 py-3">
          <h2 className="text-sm font-medium text-foreground">
            {statusFilter !== 'all'
              ? t('detail.recipientsTitleFiltered', {
                  count: filteredRecipients.length,
                  total: recipients.length,
                })
              : t('detail.recipientsTitle', { count: filteredRecipients.length })}
          </h2>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border hover:bg-transparent">
                  <TableHead className="text-muted-foreground">{t('detail.thContact')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('detail.thPhone')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('detail.thStatus')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('detail.thSent')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('detail.thDelivered')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('detail.thRead')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('detail.thError')}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                  ...
                  return (
                    <TableRow key={recipient.id} className="border-border">
                      <TableCell className="font-medium text-foreground">
                        {recipient.contact?.name ?? t('detail.unknownContact')}
                      </TableCell>
