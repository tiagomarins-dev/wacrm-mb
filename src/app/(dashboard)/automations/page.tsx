...
export default function AutomationsPage() {
  ...
  async function duplicate(a: Automation) {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? t("toastDuplicateFailed"))
      return
    }
  ...
  async function confirmDelete() {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? t("toastDeleteFailed"))
      return
    }
            ...
            {TEMPLATE_ORDER.map((slug) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-sm font-semibold text-foreground">{t(tpl.nameKey)}</div>
                  <p className="mt-1 text-xs text-muted-foreground">{t(tpl.descKey)}</p>
                </button>
              )
            })}
          </div>
      ...
      <div className="flex items-center gap-4 p-4">
        ...
        >
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>{t("lastRun", { relative: formatRelative(automation.last_executed_at, t) })}</span>
          </div>
        </button>
